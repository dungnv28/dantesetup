...
# Function to add a new proxy user
add_proxy_user() {
    local user=$1
    local password=$2
    ...
    echo "New user $user added!"
    update_proxy_file
}

# Function to remove a proxy user
remove_proxy_user() {
    local user=$1
...
if [[ -e /etc/sockd.conf ]]; then
    while : ; do
        ...
        case $option in
            ...
            echo " "
            read -p "Please enter the name for new proxy user: " -e usernew
            echo " "
    ...
    for i in $(seq 1 $numofproxy); do
        add_proxy_user "${user[$i]}" "${password[$i]}"
    done
